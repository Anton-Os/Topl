...
static void event_character_swap() {
	static unsigned swapCount = 0;
	if (swapCount % 3 == 0); // character swap
	else if (swapCount % 3 == 1); // angel swap
	else; // demon swap
	swapCount++;
}
...
void Playground_App::createScene_Details() {
	...
	Rasteron_Image* image = createMappedImg({ 4096, 4096 }, callback_gradient);
...
void Playground_App::init() {
	...
	_ticker.addPeriodicEvent(5000, event_character_swap);
