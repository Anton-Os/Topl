...
void Topl_Demo::updatePipelines(){
    float timeElapse = (float)timeline.dynamic_ticker.getAbsSecs();
    Vec3f scroll = (*camera.getPos() * -Topl_Demo::speed); // + Vec3f({ sin((*camera.getRot)[0]), cos((*camera.getRot)[0]), 0.0F }); // TODO: Include rotation
