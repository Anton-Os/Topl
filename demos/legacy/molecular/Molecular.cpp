...
void Molecular_Demo::onAnyKey(keyboard_t key){
    ...
    if((Topl_Demo::shaderMode / 10) % 3 == 0) targetLight = &skyLight;
    else if((Topl_Demo::shaderMode / 10) % 3 == 1) targetLight = &flashLight;
    ...
    if((Topl_Demo::shaderMode / 10) % 3 == 0) _beamsVShader.setLight(LIGHT_Sky, *targetLight);
    else if((Topl_Demo::shaderMode / 10) % 3 == 1) _beamsVShader.setLight(LIGHT_Flash, *targetLight);
    ...
    for(unsigned m = 0; m < 3; m++)
        for(unsigned c = 0; c < MOLECULAR_CONSTRUCTS; c++) {
            constructs[m][c].shift(Vec3f({ 
                ((float)rand() / (float)RAND_MAX - 0.5F) * MOLECULAR_SPREAD,
                ((float)rand() / (float)RAND_MAX - 0.5F) * MOLECULAR_SPREAD,
    ...
    for(unsigned m = 0; m < 3; m++)
        for (unsigned c = 0; c < MOLECULAR_CONSTRUCTS; c++) { // constructs[m][c].rotate({ 0.0F, 0.0F, (float)frameTime * 0.0000025F });
            ...
            for (unsigned o = 0; o < constructs[m][c].getActorCount(); o++) {
                ...
                for (unsigned t = 0; t < 4; t++) { // multiple draw calls for larger scene
                    actor->setPos(actorPos + Vec3f({ (t % 2 == 0)? 1.0F : -1.0F, (t % 2 == 1)? 1.0F : -1.0F, 0.0F }));
                    _renderer->update(actor);
                    _renderer->draw(actor);
                }
