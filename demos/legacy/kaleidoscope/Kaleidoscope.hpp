...
struct Kaleidoscope_Demo : public Topl_Demo {
    Kaleidoscope_Demo(const char* execPath) : Topl_Demo(execPath, "Kaleidoscope") {}
    Kaleidoscope_Demo(const char* execPath, BACKEND_Target backend) : Topl_Demo(execPath, "Kaleidoscope", backend){}
    ...
    Topl_Sampler_3D volume = Topl_Sampler_3D(KALEIDOSCOPE_SLICES);
#endif
private:
