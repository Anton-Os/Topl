...
void Cubegate_Demo::onAnyKey(keyboard_t key) {
    switch (key) {
        case 'a': worldCamera.updatePos({ -Topl_Demo::speed, 0.0F, 0.0F }); break;
        case 's': worldCamera.updatePos({ 0.0F, -Topl_Demo::speed, 0.0F }); break;
        case 'd': worldCamera.updatePos({ Topl_Demo::speed, 0.0F, 0.0F }); break;
        case 'w': worldCamera.updatePos({ 0.0F, Topl_Demo::speed, 0.0F }); break;
    }
}
...
void Cubegate_Demo::preloop() {
    ...
    if (_renderer->getFrameCount() / 60 > 300) {
        ...
        renderScene(&worldScenes[0], _coloredPipeline, Topl_Demo::shaderMode);
        frontFaceTex = _renderer->frame();
        renderScene(&worldScenes[1], _beamsPipeline, Topl_Demo::shaderMode);
        backFaceTex = _renderer->frame();
        renderScene(&worldScenes[2], _fieldPipeline, Topl_Demo::shaderMode);
        topFaceTex = _renderer->frame();
        renderScene(&worldScenes[3], _patternPipeline, Topl_Demo::shaderMode);
        bottomFaceTex = _renderer->frame();
        renderScene(&worldScenes[4], _materialPipeline, Topl_Demo::shaderMode);
        leftFaceTex = _renderer->frame();
        renderScene(&worldScenes[5], _effectPipeline, Topl_Demo::shaderMode);
        rightFaceTex = _renderer->frame();

        Topl_Sampler_2D* cubeGateTextures[6] = { &frontFaceTex, &backFaceTex, &topFaceTex, &bottomFaceTex, &leftFaceTex, &rightFaceTex };
...
void Cubegate_Demo::init() {
    ...
    for (unsigned c = 0; c < CUBEGATE_COUNT; c++) cubeGates[c].configure(&scene);

    _renderer->buildScene(&scene);
#ifdef TOPL_ENABLE_TEXTURES
    for (unsigned c = 0; c < CUBEGATE_COUNT; c++) {
...
void Cubegate_Demo::loop(double frameTime) {
    ...
    for (unsigned c = 0; c < CUBEGATE_COUNT; c++) { //cubeGates[c].rotate({ (c % 3 == 2)? f : 0.0f , (c % 3 == 1)? f : 0.0f , (c % 3 == 0)? f : 0.0f });
        float scaleFactor = cos(t * (c + 1)) * ((CUBEGATE_COUNT - c) * 0.1F);
        cubeGates[c].rotate({ 0.0f, 0.0f, f });
        cubeGates[c].scale(Vec3f({ 1.0F + scaleFactor, 1.0F + scaleFactor, 1.0F + scaleFactor }));
    }
