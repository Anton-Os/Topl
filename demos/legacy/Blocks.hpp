...
namespace App {
	...
	Geo_Chain_Params chainProps = Geo_Chain_Params(Vec3f({ 0.33f, 0.0, 0.0 })); // stretches along X axis only
	Geo_Chain chain("chain", &scene, &chainGeo, chainProps, 9);
	Geo_Actor gridGeo = Geo_Actor((const Geo_RenderObj*)&hex1);
	Geo_Grid_Params gridProps = Geo_Grid_Params(std::make_pair(3, 0.4f));
