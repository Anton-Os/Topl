...
void Animatrix_Demo::init(){
    ...
    Topl_Demo::camera.setProjMatrix(Topl_Projection(PROJECTION_Perspective, 4.0F).genProjMatrix());
...
void Animatrix_Demo::loop(double frameTime){
    ...
    renderScene(&scene3D, _beamsPipeline, Topl_Demo::shaderMode); 
}

void Animatrix_Demo::onAnyKey(keyboard_t key){
#ifdef TOPL_ENABLE_TEXTURES
    switch(tolower(key)){
        case 'd': for(unsigned p = 0; p < ANIMATRIX_PUPPETS; p++) scene2D.addForce(puppets[p].getGeoActor(PUPPET_Body)->getName(), Vec3f({ 1.0F, 0.0F, 0.0F }) * Topl_Demo::speed * 5000.0F); break;
        case 'w': for(unsigned p = 0; p < ANIMATRIX_PUPPETS; p++) scene2D.addForce(puppets[p].getGeoActor(PUPPET_Body)->getName(), Vec3f({ 0.0F, 1.0F, 0.0F }) * Topl_Demo::speed * 5000.0F); break;
        case 'a': for(unsigned p = 0; p < ANIMATRIX_PUPPETS; p++) scene2D.addForce(puppets[p].getGeoActor(PUPPET_Body)->getName(), Vec3f({ -1.0F, 0.0F, 0.0F }) * Topl_Demo::speed * 5000.0F); break;
        case 's': for(unsigned p = 0; p < ANIMATRIX_PUPPETS; p++) scene2D.addForce(puppets[p].getGeoActor(PUPPET_Body)->getName(), Vec3f({ 0.0F, -1.0F, 0.0F }) * Topl_Demo::speed * 5000.0F); break;
    }
#endif
}
