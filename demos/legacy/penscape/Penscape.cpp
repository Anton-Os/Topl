...
void Penscape_Demo::createBrushes() {
    ...
    for (unsigned b = 0; b < PENSCAPE_SEGMENTS; b++)
        for (unsigned a = 0; a < 3 * 2; a++) {
            ...
            trigMeshBrushes[a % 3]->rotate(getTurnTransform(PENSCAPE_ROT));
            ...
            quadMeshBrushes[a % 3]->rotate(getTurnTransform(PENSCAPE_ROT));
            ...
            hexMeshBrushes[a % 3]->rotate(getTurnTransform(PENSCAPE_ROT));
            ...
            circleMeshBrushes[a % 3]->rotate(getTurnTransform(PENSCAPE_ROT));
...
void Penscape_Demo::init(){
    ...
    Topl_Demo::camera.setProjMatrix(Topl_Projection(PROJECTION_Perspective, 5.0F).genProjMatrix());
...
void Penscape_Demo::loop(double frameTime){
    float f = _renderer->getFrameCount() * 0.01F;
    ...
    for (unsigned t = 0; t < brushSegments; t++) {
        ...
        for (unsigned a = 0; a < 3; a++)
            for (unsigned s = 0; s < 3; s++) {
                if(t == 0) brushes3D[a][s].setRot({ -f, f, f });
