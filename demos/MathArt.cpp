...
static void onAnyKey(char key){
    if(isdigit(key)){
        switch(key - '0'){
            ...
            case 5: _DEMO->construct.rotate({ 0.1F, 0.0F, 0.0F }); break;
            case 6: _DEMO->construct.rotate({ -0.1F, 0.0F, 0.0F }); break;
            case 7: _DEMO->construct.rotate({ 0.0F, 0.1F, 0.0F }); break;
            case 8: _DEMO->construct.rotate({ 0.0F, -0.1F, 0.0F }); break;
            case 9: _DEMO->construct.rotate({ 0.0F, 0.0F, 0.1F }); break;
            case 0: _DEMO->construct.rotate({ 0.0F, 0.0F, -0.1F }); break;
...
void MathArt_Demo::loop(double frameTime){
    for(unsigned s = 0; s < construct.getActorCount(); s++) 
        construct.getGeoActor(s)->updateRot(Vec3f({ construct.getSpinFactor(s), 0.0F, 0.0F }));
