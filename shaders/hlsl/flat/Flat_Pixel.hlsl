...
float4 main(PS_INPUT input, uint primID : SV_PrimitiveID) : SV_TARGET {
	...
	else if(mode % 10 == FLAT_DIRECTIONAL) return float4((input.pos.x / 2.0), (input.pos.y / 2.0) , (input.pos.z / 2.0), alpha); // Troubleshoot this!
	...
	else if(mode % 10 == FLAT_TANGENT) return float4(input.tangent, 1.0);
	...
	else if(mode % 10 == FLAT_RANDOM) return float4(sin(-look_pos.y), sin(look_pos.x), cos(look_pos.x + look_pos.y), alpha);
