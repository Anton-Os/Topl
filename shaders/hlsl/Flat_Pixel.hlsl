...
float4 main(PS_INPUT input, uint primID : SV_PrimitiveID) : SV_TARGET {
	...
	else if(mode % 10 == FLAT_DIRECTIONAL) return float4((input.pos.x / 2) + 0.5, (input.pos.y/ 2) + 0.5, (input.pos.z / 2) + 0.5, alpha);
	else if(mode % 10 == FLAT_COORD) return float4((input.pos.x + cam_pos.x) * 2.0 + 0.5, (input.pos.y + cam_pos.y) * 2.0 + 0.5, input.pos.z * 2.0 + 0.5, alpha);
