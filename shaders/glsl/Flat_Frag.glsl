...
void main() {
	...
	if(mode == -1) // indexing mode
		...
		outColor = vec4(abs(cam_pos.x - pos.x), abs(cam_pos.y - pos.y), abs(cam_pos.z - pos.z), 1.0);
	else if(mode % 10 == 5) // angular mode
		outColor = vec4(atan((pos.y - offset.y) / (pos.x - offset.x)), atan((pos.y - offset.y) / (pos.z - offset.z)), atan((pos.x - offset.x) / (pos.z - offset.z)), 1.0);
	// 	outColor = color * vec4(scale.x / (pos.x - offset.x), scale.y / (pos.y - offset.y), scale.z / (pos.z - offset.z), 1.0);
		...
		outColor = getRandColor(primID);
		// outColor = getRandColor(uint(floor((vertex_color.r * vertex_color.g * vertex_color.b) * 10.0)));
	else if(mode % 10 == 9) // experimental mode
		outColor = vec4(sin(vertex_color.r * id), cos(vertex_color.g * primID), tan(vertex_color.b * (id / primID)), 1.0);
