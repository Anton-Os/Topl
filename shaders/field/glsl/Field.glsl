vec3 field1(vec3 ctrlPoint, vec3 coords){
	return vec3(abs(ctrlPoint.x - coords.x), abs(ctrlPoint.y - coords.y), abs(ctrlPoint.z - coords.z)) * 0.25F;
}

vec3 field2(vec3 ctrlPoint, vec3 coords){
// vec3 field2(uint ctrlIndices[3], vec3 coords){
	return vec3(abs(ctrlPoint.x - coords.x), abs(ctrlPoint.y - coords.y), abs(ctrlPoint.z - coords.z)) * 0.5F;
}

vec3 field3(vec3 ctrlPoint, vec3 coords){
	return vec3(abs(ctrlPoint.x - coords.x), abs(ctrlPoint.y - coords.y), abs(ctrlPoint.z - coords.z)) * 0.75F;
}

vec3 field4(uint ctrlIdx, vec3 coords){
	vec3 ctrlPoint = ctrlPoints[ctrlIdx];
	return vec3(abs(ctrlPoint.x - coords.x), abs(ctrlPoint.y - coords.y), abs(ctrlPoint.z - coords.z)) * 1.0F;
}
