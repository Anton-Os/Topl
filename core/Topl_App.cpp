...
Topl_Factory::~Topl_Factory(){
    ...
    if(GL4_pipelines != nullptr){
        for(unsigned p = 0; p < GL4_pipeIndex; p++)
            delete(*(GL4_pipelines + p));
        free(GL4_pipelines);
    }

    if(Drx11_pipelines != nullptr){
        for(unsigned p = 0; p < Drx11_pipeIndex; p++)
            delete(*(Drx11_pipelines + p));
...
Topl_Renderer* Topl_Factory::genRenderer(APP_Backend backend, Platform* platform){
    switch(backend){
    case APP_OpenGL_4:
        if(GL4_renderer == nullptr) 
            GL4_renderer = new Topl_Renderer_GL4(platform->getParentWindow());
        return (Topl_Renderer*)GL4_renderer;
    case APP_DirectX_11:
        if(Drx11_renderer == nullptr)
            Drx11_renderer = new Topl_Renderer_Drx11(platform->getParentWindow());
