...
public:
    ...
    virtual Topl_Sampler_2D frame() = 0;
...
protected:
    ...
    std::map<const Topl_Sampler_Target*, std::string> _texTagMap; // tracks textures to refresh
    ...
    Topl_Sampler_2D _frameImage; // internal frame container
private:
    virtual void init(NATIVE_WINDOW window) = 0;
	virtual void swapBuffers(double frameTime) = 0;
#ifdef TOPL_ENABLE_TEXTURES
    virtual void attachTex(const Topl_Sampler_2D* image, unsigned renderID) { attachTexAt(image, renderID, 0); } // attaches to default binding
    virtual void attachTexAt(const Topl_Sampler_2D* image, unsigned renderID, unsigned binding) = 0;
    virtual void attachTex3D(const Topl_Sampler_3D* volumeTex, unsigned renderID) = 0;
#endif
