...
Topl_Sampler_2D Topl_Renderer_GL4::frame() {
	...
	_frameImage = Topl_Sampler_2D(mirrorImgOp(flipImage));

	RASTERON_DEALLOC(stageImage); RASTERON_DEALLOC(flipImage);
	return _frameImage;
}

void Topl_Renderer_GL4::attachTexAt(const Topl_Sampler_2D* image, unsigned renderID, unsigned binding) {
...
void Topl_Renderer_GL4::attachTex3D(const Topl_Sampler_3D* volumeTex, unsigned renderID) {
	GLuint textureTarget = *(_textureSlots + _textureIndex);
	_textureIndex++; // increments to next available slot
	...
	const Topl_Sampler_2D* volumeTexImage = volumeTex->getVolumeImg();
