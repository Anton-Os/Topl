...
public:
	Topl_Renderer_GL4(NATIVE_PLATFORM_CONTEXT* context) : Topl_Renderer(context) {
        ...
        _clearColors = Vec4f({ 0.25F, 1.0F, 0.25F, 1.0F }); // green tint
	}
	~Topl_Renderer_GL4();
	...
	Topl_Sampler_2D frame() override;
#endif
#ifndef __ANDROID__
...
#ifdef TOPL_ENABLE_TEXTURES
    void attachTexAt(const Topl_Sampler_2D* imageTex, unsigned renderID, unsigned binding) override;
	void attachTex3D(const Topl_Sampler_3D* volumeTex, unsigned id) override;
#endif
