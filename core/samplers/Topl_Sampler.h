...
struct Topl_Sampler_Target { // Refresh State
    Topl_Sampler_Target(TEX_Frmt f){ 
		format = f;
...
struct Topl_Sampler_2D : public Topl_Sampler_Target {
    Topl_Sampler_2D() : Topl_Sampler_Target(TEX_2D){} // Empty Constructor
	// Topl_Sampler_2D(const char* name, unsigned width, unsigned height) : Topl_Sampler_Target(TEX_2D){}
#ifdef TOPL_ENABLE_TEXTURES // required library for loading images
    Topl_Sampler_2D(Rasteron_Image* refImage) : Topl_Sampler_Target(TEX_2D){
		setImage(refImage);
		RASTERON_DEALLOC(refImage);
	} // Custom Constructor
    // ~Topl_Sampler_2D(){ cleanup(); }
...
struct Topl_Sampler_Array : public Topl_Sampler_2D {
#ifndef RASTERON_H
    Topl_Sampler_Array() : Topl_Sampler_2D(){}
#else
    Topl_Sampler_Array() : Topl_Sampler_2D(){ queue = RASTERON_QUEUE_ALLOC("arrayTex", RASTERON_SIZE(SAMPLER_HEIGHT, SAMPLER_WIDTH), 8); }
    Topl_Sampler_Array(unsigned short count) : Topl_Sampler_2D(){ queue = RASTERON_QUEUE_ALLOC("arrayTex", RASTERON_SIZE(SAMPLER_HEIGHT, SAMPLER_WIDTH), count); }
...
#define SAMPLER_VOLUME_BACKGROUND 0xFFFFFFFF
#define SAMPLER_COLOR_QUEUE nullptr

struct Topl_Sampler_3D : public Topl_Sampler_Target {
    Topl_Sampler_3D() : Topl_Sampler_Target(TEX_3D), width(SAMPLER_WIDTH), height(SAMPLER_HEIGHT), depth(SAMPLER_WIDTH) {} // Empty Constructor
#ifdef TOPL_ENABLE_TEXTURES
    Topl_Sampler_3D(unsigned s) : width(s), height(s), depth(s), Topl_Sampler_Target(TEX_3D){ setData(SAMPLER_COLOR_QUEUE); } // Matching Lengths
    Topl_Sampler_3D(unsigned w, unsigned h, unsigned z) : width(w), height(h), depth(z), Topl_Sampler_Target(TEX_3D){ setData(SAMPLER_COLOR_QUEUE); } // Custom Lengths
    Topl_Sampler_3D(Rasteron_Queue* q) : width(queue_getImg(queue, 0)->width), height(queue_getImg(queue, 0)->height), depth(queue->frameCount), Topl_Sampler_Target(TEX_3D){
		assert(width == height);
		setData(q);
	}
	/* Topl_Sampler_3D(std::string directory) {} */ // TODO:: Load all images in directory and add to queue
	
	Rasteron_Image* getSlice(unsigned d) const { return (d < depth) ? queue_getImg(queue, d) : nullptr;}
    const Topl_Sampler_2D* getVolumeImg() const { return &volumeTexImg; }
...
private:
    ...
    Topl_Sampler_2D volumeTexImg; // conversion from 3D texture into underlying queue
