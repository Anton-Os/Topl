...
#include "samplers/Topl_Sampler_2D.h"
#include "samplers/Topl_Sampler_UI.h"
...
public:
	...
	void addTexture(const std::string& name, const Topl_Sampler_2D* image);
	void addTexture(const Topl_Sampler_2D* image){ for(auto t = _geoActors.begin(); t != _geoActors.end(); t++) addTexture((*t)->getName(), image); }
	void addArrayTex(const std::string& name, const Topl_Sampler_Array* arrayTex);
	void addVolumeTex(const std::string& name, const Topl_Sampler_3D* volumeTex);
	void addVolumeTex(const Topl_Sampler_3D* volumeTex){ for(auto t = _geoActors.begin(); t != _geoActors.end(); t++) addVolumeTex((*t)->getName(), volumeTex); }
#endif
...
#ifdef TOPL_ENABLE_TEXTURES
	const Topl_Sampler_2D* getTexture(const std::string& name) const;
	const Topl_Sampler_Array* getArrayTex(const std::string& name) const;
	const Topl_Sampler_3D* getVolumeTex(const std::string& name) const;
#endif
	...
	void resolvePhysics(){ resolvePhysics(FORCE_Directional); }
...
#ifdef TOPL_ENABLE_TEXTURES
	Topl_Sampler_2D _textures[MAX_TEX_BINDINGS] = { 
		// Topl_Sampler_File(texImgPaths[0]), Topl_Sampler_File(texImgPaths[1]), Topl_Sampler_File(texImgPaths[2]), Topl_Sampler_File(texImgPaths[3]), 
		// Topl_Sampler_File(texImgPaths[4]), Topl_Sampler_File(texImgPaths[5]), Topl_Sampler_File(texImgPaths[6]), Topl_Sampler_File(texImgPaths[7]),
		Topl_Sampler_Color(RAND_COLOR()), Topl_Sampler_Color(RAND_COLOR()), Topl_Sampler_Color(RAND_COLOR()), Topl_Sampler_Color(RAND_COLOR()),
		Topl_Sampler_Color(RAND_COLOR()), Topl_Sampler_Color(RAND_COLOR()), Topl_Sampler_Color(RAND_COLOR()), Topl_Sampler_Color(RAND_COLOR())
	};
	std::map<Geo_Actor*, const Topl_Sampler_2D*> _textureMap; // associates geometry actor to single texture // TODO: Reverse order
	std::map<Geo_Actor*, const Topl_Sampler_Array*> _arrayTexMap; // associates geometry actor to multiple 2D textures // TODO: Reverse order
	std::map<Geo_Actor*, const Topl_Sampler_3D*> _volumeTexMap; // associates geometry actor to volumeTextric texture // TODO: Reverse order
#endif
